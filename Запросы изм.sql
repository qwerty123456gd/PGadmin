...
/*вывести всю информацию о клиенте случайном клиенте(паспорт, номера телефонов, тариф и его стоимость)*/
/*Select klients.id, last_name, first_name, surname, passport_series, passport_number, phone_number, tname, tarifs.tarif_price_rub
from klients, number_of_telephone, klients_number, Tarifs, tar_in_tel
where klients.id = klients_number.klient_id and number_of_telephone.id = klients_number.number_id
and number_of_telephone.id = tar_in_tel.tel_id and Tarifs.id = tar_in_tel.tar_id and klients.id = 1*/

/*вывести информацию о количестве предоставленных гигабайт в июле месяце и на какую стоимость*/
